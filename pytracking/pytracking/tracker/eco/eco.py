...
class ECO(BaseTracker):
    ...
    def __init__(self, params):
        ...
        if self.params.get('preload_features_on_create', False):
            self.initialize_features()
        if self.params.get('warmup_on_create', False):
            self.warmup_runtime()

    def warmup_runtime(self):
        if getattr(self, 'runtime_warmed', False):
            return

        warmup_size = int(self.params.get('warmup_image_sz', 128))
        device = 'cuda' if self.params.use_gpu and torch.cuda.is_available() else 'cpu'

        with torch.no_grad():
            warm_patch = torch.zeros(1, 3, warmup_size, warmup_size)
            for feature in self.params.features.features:
                feature_outputs = feature.get_feature(warm_patch)
                if isinstance(feature_outputs, torch.Tensor):
                    feature_outputs = [feature_outputs]
                for feat in feature_outputs:
                    if feat.numel() == 0:
                        continue
                    feat = feat.to(device)
                    _ = fourier.cfft2(feat)

            if device == 'cuda':
                _ = torch.svd(torch.eye(8, device=device))
                torch.cuda.synchronize()

        self.runtime_warmed = True

    def initialize_features(self):
        if not getattr(self, 'features_initialized', False):
            if self.params.use_gpu and torch.cuda.is_available():
                torch.backends.cudnn.benchmark = self.params.get('cudnn_benchmark', True)
                allow_tf32 = self.params.get('allow_tf32', True)
                if hasattr(torch.backends, 'cuda') and hasattr(torch.backends.cuda, 'matmul'):
                    torch.backends.cuda.matmul.allow_tf32 = allow_tf32
                if hasattr(torch.backends, 'cudnn') and hasattr(torch.backends.cudnn, 'allow_tf32'):
                    torch.backends.cudnn.allow_tf32 = allow_tf32
                if self.params.has('matmul_precision') and hasattr(torch, 'set_float32_matmul_precision'):
                    torch.set_float32_matmul_precision(self.params.matmul_precision)
    ...
    def get_active_scale_factors(self):
        refresh_interval = int(self.params.get('scale_refresh_interval', 1))
        if refresh_interval <= 1:
            return self.params.scale_factors

        score_threshold = float(self.params.get('scale_confidence_threshold',
                                                self.params.get('adaptive_score_threshold', 0.25)))
        if getattr(self, 'last_max_score', float('inf')) < score_threshold:
            return self.params.scale_factors

        if (self.frame_num - 1) % refresh_interval == 0:
            return self.params.scale_factors

        intermediate_scale_factors = self.params.get('intermediate_scale_factors', None)
        if intermediate_scale_factors is None:
            return torch.ones(1, dtype=self.params.scale_factors.dtype, device=self.params.scale_factors.device)
        if isinstance(intermediate_scale_factors, (list, tuple)):
            return torch.tensor(intermediate_scale_factors, dtype=self.params.scale_factors.dtype,
                                device=self.params.scale_factors.device)
        return intermediate_scale_factors

    def get_tracking_diagnostics(self, translation_vec, scale_change_factor, max_score: float):
        motion_threshold = float(self.params.get('adaptive_motion_threshold', 0.06))
        scale_threshold = float(self.params.get('adaptive_scale_threshold', 0.01))
        score_threshold = float(self.params.get('adaptive_score_threshold', 0.25))

        target_extent = max(float(self.target_sz.mean().item()), 1.0)
        norm_motion = float(translation_vec.norm().item()) / target_extent
        scale_delta = abs(float(scale_change_factor) - 1.0)
        stable_frame = norm_motion < motion_threshold and scale_delta < scale_threshold and max_score > score_threshold

        return stable_frame, norm_motion, scale_delta

    def should_update_filter(self, translation_vec, scale_change_factor, max_score: float) -> bool:
        if not self.params.get('adaptive_update', False):
            return self.frame_num % self.params.train_skipping == 1

        min_interval = int(self.params.get('adaptive_min_interval', self.params.train_skipping))
        max_interval = int(self.params.get('adaptive_max_interval', max(min_interval, self.params.train_skipping * 2)))

        frames_since_update = self.frame_num - getattr(self, 'last_train_frame', 1)
        stable_frame, _, _ = self.get_tracking_diagnostics(translation_vec, scale_change_factor, max_score)

        if frames_since_update >= max_interval:
            return True
        if frames_since_update < min_interval:
            return False
        return not stable_frame

    def get_train_cg_iters(self, translation_vec, scale_change_factor, max_score: float) -> int:
        if not self.params.get('adaptive_cg', False):
            return self.params.CG_iter

        stable_frame, _, _ = self.get_tracking_diagnostics(translation_vec, scale_change_factor, max_score)
        if stable_frame:
            return int(self.params.get('stable_CG_iter', max(1, self.params.CG_iter // 2)))
        return int(self.params.get('recovery_CG_iter', self.params.CG_iter))
